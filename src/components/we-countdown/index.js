export default Component({
  ...
  methods: {
    ...
    init() {
      ...
      this.endTimeMs = now + this.computeTime;
    ...
    setCountdownTimeItems(computeTimeMs) {
      this.computeTime = parseInt(Math.ceil(computeTimeMs));
    ...
    getTime(leftseconds, format) {
      let d = leftseconds;
      let [v, s, m, h] = [1000, 60, 60, 24].map(unit => {
        let num = d % unit;
        d = Math.floor(d / unit);
        return num;
      });
      ...
      if (leftseconds > 1 && format.indexOf('v') === -1) {
        v += s * 1000;
      }
      ...
      return {
        ...
        vv: this.formatTime(Math.floor((v) / 10)),
        d,
        h,
        m,
        s,
    ...
    onPageShow() {
      ...
      if (this.format && this.endTimeMs) {
        this.computeTime = parseInt((this.endTimeMs - now));
        this.initCountdown();
      }
    },
